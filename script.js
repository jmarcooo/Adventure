...
let selectedTargetId = null; // NEW: Tracks the player's manual target
    ...
    if (savedData) {
        try {
            ...
            if (parsed.player) {
                ...
                updateUI(); renderHeroSelection(); renderGearMenu(); renderStagesMenu();
                document.getElementById('global-modal-backdrop').style.display = 'none';
                document.getElementById('settings-modal').style.display = 'none';
                showNotification("📂 Game Loaded!");
            }
        } catch (e) { console.error("Save file error:", e); showNotification("⚠️ Error loading save!"); }
    } else { showNotification("No save file found."); }
}
...
function showGearModal(item, source, key) {
    ...
    if (source === 'inventory') { actionBtn.innerText = 'EQUIP'; actionBtn.onclick = () => { equipItem(key, item.slot); document.getElementById('gear-details-modal').style.display = 'none'; document.getElementById('global-modal-backdrop').style.display='none';}; } 
    else { actionBtn.innerText = 'UNEQUIP'; actionBtn.onclick = () => { unequipItem(key); document.getElementById('gear-details-modal').style.display = 'none'; document.getElementById('global-modal-backdrop').style.display='none';}; }
...
function getPlayerStats() {
    ...
    let lvlBonusPAtk = (hStats.level - 1) * (hero.pAtkGrowth || 0); let lvlBonusMAtk = (hStats.level - 1) * (hero.mAtkGrowth || 0); 
    let lvlBonusPDef = (hStats.level - 1) * (hero.pDefGrowth || 0); let lvlBonusMDef = (hStats.level - 1) * (hero.mDefGrowth || 0);
...
function getTotalDamage() {
    let stats = getPlayerStats(); let hero = heroData[player.currentHero];
    let baseP = stats.pAtk; let baseM = stats.mAtk; 
    ...
    if (hero && hero.attackType === 'magical') { baseM += player.bonusDamage; } else { baseP += player.bonusDamage; }
...
function updateGearStatsPanel() {
    ...
    let stats = getPlayerStats(); let maxH = getMaxHealth(); let hero = heroData[player.currentHero];
    ...
    html += `<div class="equip-stat-row"><span>Max HP</span> <span>${maxH}</span></div>`; html += `<div class="equip-stat-row"><span style="color:${hero.attackType === 'magical' ? '#555' : '#fff'}">P.Atk</span> <span style="color:${hero.attackType === 'magical' ? '#555' : '#fff'}">${stats.pAtk}</span></div>`; html += `<div class="equip-stat-row"><span style="color:${hero.attackType === 'physical' ? '#555' : '#fff'}">M.Atk</span> <span style="color:${hero.attackType === 'physical' ? '#555' : '#fff'}">${stats.mAtk}</span></div>`; html += `<div class="equip-stat-row"><span>P.Def</span> <span>${stats.pDef}</span></div>`; html += `<div class="equip-stat-row"><span>M.Def</span> <span>${stats.mDef}</span></div>`; html += `<div class="equip-stat-row"><span>Atk Spd</span> <span>${stats.atkSpd.toFixed(2)}</span></div>`; html += `<div class="equip-stat-row"><span>Speed</span> <span>${stats.spd}</span></div>`; html += `<div class="equip-stat-row"><span>Crit %</span> <span>${(stats.crit * 100).toFixed(1)}%</span></div>`; html += `<div class="equip-stat-row"><span>Evasion %</span> <span>${(stats.evasion * 100).toFixed(1)}%</span></div>`; html += `<div class="equip-stat-row"><span>Luck</span> <span>${stats.luck}</span></div>`; 
...
function renderForge() {
    let container = document.getElementById('forge-recipes-container'); container.innerHTML = '';
    let matsContainer = document.getElementById('forge-mats-container'); matsContainer.innerHTML = '';

    let hasMats = false;
    for(let matId in player.materials) {
        if(player.materials[matId] > 0) {
            hasMats = true; let mData = materialsData[matId] || { icon: '📦', name: 'Unknown' };
    ...
    recipesData.forEach(recipe => {
        let canCraft = true; let costHtml = `<div style="font-size:0.8rem; margin-top:5px; display:flex; gap:5px; flex-wrap:wrap;">`;
        ...
        for(let matId in recipe.materials) {
            let needed = recipe.materials[matId]; let owned = player.materials[matId] || 0;
            if (owned < needed) canCraft = false; let mData = materialsData[matId] || { icon: '📦' };
...
function craftItem(recipeId) {
    let recipe = recipesData.find(r => r.id === recipeId); if(!recipe) return;
    player.gold -= recipe.goldCost; for(let matId in recipe.materials) { player.materials[matId] -= recipe.materials[matId]; }
...
function dropMaterial(matId, amount, unitId) {
    if (!player.materials) player.materials = {}; if (!player.materials[matId]) player.materials[matId] = 0;
    player.materials[matId] += amount;
    
    if (!runStats.materialsGained) runStats.materialsGained = {}; if (!runStats.materialsGained[matId]) runStats.materialsGained[matId] = 0;
    runStats.materialsGained[matId] += amount;
    
    let mat = materialsData[matId];
    if (mat) { spawnFloatingText(`enemy-${unitId}`, `+${amount} ${mat.icon}`, "float-heal"); showNotification(`Looted: ${amount}x ${mat.icon} ${mat.name}`); }
}

function renderMaterialRecap(containerId) {
    let container = document.getElementById(containerId); if (!container) return; container.innerHTML = '';
    let hasMats = false;
    ...
    if (!hasMats) { container.innerHTML = '<span style="color:#7f8c8d; font-size:0.9rem;">No materials found.</span>'; }
}
...
function processTimeEffects(unit) {
        ...
        if (eff.duration !== undefined) {
            ...
            if (eff.tickTimer >= 1000) {
                ...
                if (hpDmgPct > 0) {
                    ...
                    if (unit === player) { player.currentHealth -= mutatorDmg; if (player.currentHealth <= 0) triggerGameOver("Choked by Toxic Air"); } else { unit.hp -= mutatorDmg; animateHit(unit.id, mutatorDmg, false); }
                }

                if (eff.type === 'poison') {
                    let dmg = Math.max(1, Math.floor(maxH * 0.05));
                    if (unit === player) { player.currentHealth -= dmg; spawnFloatingText('player-combat-area', "POISON -" + dmg, "float-enemy-dmg"); if (player.currentHealth <= 0) triggerGameOver("Succumbed to poison"); } else { unit.hp -= dmg; animateHit(unit.id, dmg, false); }
                } 
                else if (eff.type === 'burn') {
                    let dmg = Math.max(1, Math.floor(maxH * 0.08));
                    if (unit === player) { player.currentHealth -= dmg; spawnFloatingText('player-combat-area', "BURN -" + dmg, "float-enemy-dmg"); if (player.currentHealth <= 0) triggerGameOver("Burnt to a crisp"); } else { unit.hp -= dmg; animateHit(unit.id, dmg, false); }
                }
            ...
            if (eff.duration <= 0) {
                if (eff.type === 'doom') {
                    if (unit === player) { player.currentHealth = 0; spawnFloatingText('player-combat-area', "DOOMED!", "float-enemy-dmg"); triggerGameOver("Erased from existence by Doom"); } else { unit.hp = 0; finalizeKill = true; animateHit(unit.id, 9999, false); spawnFloatingText(`enemy-${unit.id}`, "DOOMED!", "float-crit"); }
                }
    ...
    if (finalizeKill) {
        unit.activeEffects = []; let sCont = document.getElementById(`enemy-status-${unit.id}`); if (sCont) sCont.innerHTML = '';
        ...
        if (unit === player) updatePlayerHealthBar(); if (uiChanged) renderStatusEffects();
    }
}
...
function playBattleStartAnimation() {
    let overlay = document.getElementById('battle-start-overlay'); let swords = document.getElementById('crossed-swords');
    if(overlay && swords) { overlay.style.display = 'flex'; swords.classList.remove('animate-swords'); void swords.offsetWidth; swords.classList.add('animate-swords'); setTimeout(() => { overlay.style.display = 'none'; }, 1000); }
}

function spawnFloatingText(targetId, text, className) {
    let targetDiv = document.getElementById(targetId); if(!targetDiv) return;
    let ft = document.createElement('div'); ft.className = `floating-text ${className}`; ft.innerText = text; let offsetX = (Math.random() * 40) - 20; ft.style.left = `calc(50% + ${offsetX}px)`; ft.style.top = '10px';
...
function summonEnemyMidBattle(enemyId, hpOverride = null) {
    ...
    let normPAtk = Math.max(0, Math.floor((template.pAtk || 0) + (stageInfo.absoluteLevel * 0.8))); let normMAtk = Math.max(0, Math.floor((template.mAtk || 0) + (stageInfo.absoluteLevel * 0.8)));
    let normPDef = Math.floor((template.pDef || 0) + (stageInfo.absoluteLevel * 0.5)); let normMDef = Math.floor((template.mDef || 0) + (stageInfo.absoluteLevel * 0.5));
    ...
    let newEnemy = { id: newId, name: template.name, maxHp: normHp, hp: normHp, pAtk: normPAtk, mAtk: normMAtk, pDef: normPDef, mDef: normMDef, exp: normExp, skill: template.skill, attackProgress: 0, activeEffects: [], isDead: false, isBoss: false, isElite: false, spd: template.spd || 10, atkSpd: template.atkSpd || 1.0 };
...
function combatTick() {
    ...
    if (!hasStatus(player, 'stun')) { player.attackProgress += (Math.max(0.1, stats.atkSpd) * gameSpeed * 2.5 * atbMult); if (player.attackProgress >= 100) { player.attackProgress = 100; actionQueue.push({ type: 'player', spd: stats.spd }); } } 
...
function startTestBattle() {
    ...
    let container = document.getElementById('enemy-container'); let textEl = document.getElementById('level-wave-text'); let mutatorEl = document.getElementById('mutator-display'); if(mutatorEl) mutatorEl.style.display = 'none'; activeMutator = null; container.innerHTML = ''; activeEnemies = [];
...
function spawnEnemyPack() {
    ...
    container.innerHTML = ''; activeEnemies = []; activeMutator = null; selectedTargetId = null;
    if(mutatorEl) mutatorEl.style.display = 'none';

    let stageInfo = getLevelAndWave();
    let biomeEnemies = enemiesData[stageInfo.biome.id];
    if (!biomeEnemies || biomeEnemies.length === 0) biomeEnemies = [{ id: 'error_slime', name: 'Slime', emoji: '👾', baseHp: 20, pAtk: 2, mAtk: 0, pDef: 1, mDef: 1, spd: 10, atkSpd: 1.0, exp: 10, skill: null }];
    ...
    if (stageInfo.isBiomeBoss) {
        ...
        let bPAtk = Math.floor((10 + (stageInfo.absoluteLevel * 1.5)) * statMult); let bMAtk = Math.floor((10 + (stageInfo.absoluteLevel * 1.5)) * statMult);
        let bPDef = Math.floor((5 + (stageInfo.absoluteLevel * 1.0)) * statMult); let bMDef = Math.floor((5 + (stageInfo.absoluteLevel * 1.0)) * statMult);
        ...
        let bPAtk = Math.floor((8 + (stageInfo.absoluteLevel * 1.2)) * statMult); let bMAtk = Math.floor((8 + (stageInfo.absoluteLevel * 1.2)) * statMult);
        let bPDef = Math.floor((5 + (stageInfo.absoluteLevel * 0.8)) * statMult); let bMDef = Math.floor((5 + (stageInfo.absoluteLevel * 0.8)) * statMult);
        ...
        for(let i = 0; i < spawnList.length; i++) {
            ...
            if (guaranteedElites > 0) { isElite = true; guaranteedElites--; } 
            else { let dynamicEliteChance = 0.02 * stageInfo.wave; if (isRuins) dynamicEliteChance *= 2; if (Math.random() < dynamicEliteChance) isElite = true; }
            ...
            let finalPAtk = isElite ? Math.floor(normPAtk * 1.5) : normPAtk; let finalMAtk = isElite ? Math.floor(normMAtk * 1.5) : normMAtk;
            let finalPDef = isElite ? Math.floor(normPDef * 1.5) : normPDef; let finalMDef = isElite ? Math.floor(normMDef * 1.5) : normMDef;
            let finalExp = isElite ? normExp * 3 : normExp;
            
            let eName = (isElite ? 'Elite ' : '') + eTemp.name; 
            let finalSpd = eTemp.spd || (isElite ? 12 : 10); let finalAtkSpd = eTemp.atkSpd || 1.0;
    ...
    activeEnemies.forEach(e => {
        e.spd = e.spd || (e.isBoss ? 15 : (e.isElite ? 12 : 10)); e.atkSpd = e.atkSpd || (e.isBoss ? 1.2 : 1.0); 
    ...
    combatants.forEach((c, index) => {
        ...
        if (c.type === 'player') {
            player.attackProgress = initialProgress; let pAtb = document.getElementById('player-atb'); if(pAtb) pAtb.style.width = Math.max(0, Math.min(100, player.attackProgress)) + '%';
        } else {
            c.ref.attackProgress = initialProgress; let atbBar = document.getElementById(`enemy-atb-bar-${c.ref.id}`); if (atbBar) atbBar.style.width = Math.max(0, Math.min(100, c.ref.attackProgress)) + '%';
        }
    });
...
function handleEnemyDeath(target, unitId, unitDiv) {
    ...
    if (unitId === selectedTargetId) selectedTargetId = null; // Clear target lock if they die
    ...
    try {
        ...
        let bossMats = Object.keys(materialsData).filter(k => materialsData[k].boss === stageInfo.biome.id);
        ...
        if (target.isBoss) {
            ...
            let stats = getPlayerStats(); let goldEarned = Math.floor(3 * (1 + (stats.luck || 0))); 
            addCurrency('gold', goldEarned);
            ...
            if (Math.random() < 0.25) { 
                let goldEarned = Math.floor(1 * (1 + (stats.luck || 0))); if (goldEarned < 1) goldEarned = 1; 
                addCurrency('gold', goldEarned); spawnLootDrop(`enemy-${unitId}`, 'gold'); 
            }
            if (Math.random() < 0.25 && genMats.length > 0) { dropMaterial(genMats[Math.floor(Math.random() * genMats.length)], 1, unitId); }
        }
        document.getElementById('run-runes-text').innerText = runStats.runes;
    } catch (e) { console.error("Loot system failed: ", e); }

    if (activeEnemies.length > 0 && activeEnemies.every(e => e.isDead)) {
        if (!waveManager.transitioning) { waveManager.transitioning = true; setTimeout(() => packDefeated(), 400); }
    }
}
...
function executePlayerAttack() {
    ...
    if (hasStatus(player, 'bleed')) {
        let bDmg = Math.floor(getMaxHealth() * 0.05); player.currentHealth -= bDmg; updatePlayerHealthBar(); spawnFloatingText('player-combat-area', "BLEED -" + bDmg, "float-enemy-dmg");
    ...
    for (let s = 0; s < strikes; s++) {
        setTimeout(() => {
            ...
            if (selectedTargetId !== null) {
                target = activeEnemies.find(e => e.id === selectedTargetId && e.hp > 0 && !e.isDead);
            }
            if (!target) { // Fallback if no target or target died
                target = activeEnemies.find(e => e.hp > 0 && !e.isDead); 
            }
            if (!target) return;
            // ---------------------------
            ...
            if (innateTrigger) {
                ...
                if (hero.innateSkill.type === 'hunter_instakill' && !target.isBoss) { dmg = target.hp; } 
                    ...
                    dmg = dmg * 3; player.currentHealth -= Math.floor(getMaxHealth() * 0.05); updatePlayerHealthBar();
            ...
            if (consumeCharge(target, 'block')) { spawnFloatingText(`enemy-${target.id}`, "BLOCKED!", "float-miss"); dmg = 0; } 
                ...
                if (target.hp > 0 && target.hp <= target.maxHp * 0.5) {
                    ...
                    if (tSkill && tSkill.trigger === 'hp_threshold') {
                        if (tSkill.enrageMultiplier && !target.hasEnraged) {
                            target.hasEnraged = true; spawnFloatingText(`enemy-${target.id}`, tSkill.floatingText || "ENRAGED!", "float-enemy-dmg");
                            target.pAtk = Math.floor((target.pAtk || target.damage || 10) * tSkill.enrageMultiplier); target.mAtk = Math.floor((target.mAtk || target.damage || 10) * tSkill.enrageMultiplier); target.atkSpd = (target.atkSpd || 1) * tSkill.enrageMultiplier;
                        }
                        if (tSkill.summonEnemyId && !target.hasSummoned) {
                            target.hasSummoned = true; spawnFloatingText(`enemy-${target.id}`, tSkill.floatingText || "SUMMON!", "float-enemy-dmg");
                            let count = tSkill.summonCount || 1; for(let i=0; i<count; i++) summonEnemyMidBattle(tSkill.summonEnemyId, tSkill.summonHpOverride);
                        }
                    }
                }
                ...
                if (player.equipment && player.equipment.weapon && player.equipment.weapon.onHit) {
                    ...
                    if (Math.random() < hitEff.chance) { applyStatus(target, hitEff.type, hitEff.duration); spawnFloatingText(`enemy-${target.id}`, `${hitEff.type.toUpperCase()}!`, "float-enemy-dmg"); }
                }

                if (defSkill && defSkill.trigger === 'defend' && defSkill.reflectPhysicalFraction && pDmg > 0) {
                    let tDmg = Math.floor(pDmg * defSkill.reflectPhysicalFraction); player.currentHealth -= tDmg; updatePlayerHealthBar(); spawnFloatingText('player-combat-area', "REFLECT -" + tDmg, "float-enemy-dmg");
...
function executeEnemyAttack(e) {
    ...
    let isMagic = skillData && skillData.isMagic; let ignoresEvasion = skillData && skillData.ignoresEvasion; let isPiercing = skillData && skillData.isPiercing;
    let skillTriggered = false;

    if (skillData) { if (skillData.trigger === 'always') skillTriggered = true; if (skillData.trigger === 'attack' && skillData.chance && Math.random() < skillData.chance) skillTriggered = true; }

    let isCrit = false;
    if (skillTriggered && skillData.floatingText === 'CRIT!') isCrit = true;
    if (consumeCharge(e, 'focused')) isCrit = true; if (consumeCharge(player, 'marked')) isCrit = true;
    ...
    if (skillTriggered) {
        ...
        if (skillData.floatingText && skillData.floatingText !== 'CRIT!') { spawnFloatingText('player-combat-area', skillData.floatingText, "float-enemy-dmg"); }
    }
    ...
    if (hasStatus(player, 'vulnerable')) incomingDmg = Math.floor(incomingDmg * 1.5); if (hasStatus(player, 'barrier')) incomingDmg = Math.floor(incomingDmg * 0.5);
    ...
    if (incomingDmg > 0) {
        for (let slotKey in player.equipment) {
            ...
            if (equippedItem && equippedItem.onHitTaken) {
                ...
                if (Math.random() < proc.chance) { applyStatus(e, proc.type, proc.duration, proc.type === 'blind'); spawnFloatingText(`enemy-${e.id}`, `${proc.type.toUpperCase()}!`, "float-enemy-dmg"); }
            }
        }
    }
    ...
    if (skillData && skillData.summonEnemyId && skillData.trigger === 'attack') {
        let aliveCount = activeEnemies.filter(en => en.hp > 0 && !en.isDead).length; let maxAllowed = skillData.maxMinionsOnBoard || 5;
        if (aliveCount < maxAllowed) { summonEnemyMidBattle(skillData.summonEnemyId, skillData.summonHpOverride); }
    }
...
function updatePlayerHealthBar() {
    ...
    if (pBar) {
        if (pPercent <= 30) pBar.style.backgroundColor = '#e74c3c'; else if (pPercent <= 50) pBar.style.backgroundColor = '#e67e22'; else if (pPercent <= 70) pBar.style.backgroundColor = '#f1c40f'; else pBar.style.backgroundColor = '#2ecc71';
    }
}
...
function packDefeated() {
    ...
    commonUpgradesData.forEach(u => {
        ...
        if (u.id === 'heal') { newDesc = `Heal 25% Max HP`; } else if (u.id === 'temp_atk') { newDesc = `+200% P/M.Atk (1 Wave)`; } else if (u.id === 'vitality') { newEffect.hp = 25 * scaleMult; newDesc = `+${newEffect.hp} Max HP`; }
        shopPool.push({ ...u, rarity: 'common', cost: 0, effect: newEffect, desc: newDesc });
    });

    runUpgradeData.forEach(u => {
        if (runStats.upgradeLevels[u.id] < u.maxLevel) { let cost = 1 + (runStats.upgradeLevels[u.id] * 2); shopPool.push({ ...u, rarity: 'uncommon', cost: cost, currentLvl: runStats.upgradeLevels[u.id] }); }
    });
...
function showUpgradeShop(shopPool) {
    document.getElementById('wave-upgrade-ui').style.display = 'flex'; document.getElementById('shop-runes-display').innerText = runStats.runes;
...
function buyRunUpgrade(upgrade) {
    ...
    if (upgrade.rarity === 'uncommon') {
        ...
        if(upgrade.id === 'p_atk') runStats.pAtk += 20; if(upgrade.id === 'm_atk') runStats.mAtk += 20;
        if(upgrade.id === 'p_def') runStats.pDef += 10; if(upgrade.id === 'm_def') runStats.mDef += 10;
        ...
        if(upgrade.id === 'splash') runStats.splashDmg += 0.10; if(upgrade.id === 'double') runStats.doubleHitChance += 0.10;
        if(upgrade.id === 'lifesteal') runStats.lifesteal += 0.05; if(upgrade.id === 'evasion') runStats.evasion += 0.05;
...
function continueToNextWave() {
    ...
    waveManager.wave++; waveManager.isUpgrading = true; spawnEnemyPack(); playBattleStartAnimation();
...
function collectRunRewards() {
    ...
    if (hStats) {
        ...
        while(hStats.exp >= hStats.expNeeded) {
            ...
            player.talentPoints++; player.maxHealth += 25; leveledUp = true;
        }
        if(leveledUp) showNotification(`🎉 Level Up! You are now Level ${hStats.level}!`);
    }
    
    document.getElementById('run-summary-ui').style.display = 'none'; document.getElementById('boss-clear-ui').style.display = 'none';
...
function generateRandomEquipment() {
    if (equipmentData && equipmentData.length > 0) {
        ...
        let statsCopy = {}; if (baseItem.stats) { for (let key in baseItem.stats) { statsCopy[key] = baseItem.stats[key]; } }
    ...
    const slots = ['head', 'body', 'legs', 'boots', 'weapon', 'leftHand', 'ring', 'amulet']; const slot = slots[Math.floor(Math.random() * slots.length)]; const prefixes = ['Rusty', 'Iron', 'Steel']; const prefix = prefixes[Math.floor(Math.random() * prefixes.length)];
    const itemTypes = { 'head': { name: 'Helm', icon: '🪖', stats: ['pDef', 'mDef', 'maxHp'] }, 'body': { name: 'Armor', icon: '👕', stats: ['pDef', 'mDef', 'maxHp'] }, 'legs': { name: 'Greaves', icon: '👖', stats: ['pDef', 'spd', 'evasion'] }, 'boots': { name: 'Boots', icon: '🥾', stats: ['spd', 'evasion', 'mDef'] }, 'weapon': { name: 'Sword', icon: '🗡️', stats: ['pAtk', 'mAtk', 'atkSpd', 'crit'] }, 'leftHand': { name: 'Off-Hand', icon: '🛡️', stats: ['pDef', 'mDef', 'maxHp', 'mAtk', 'pAtk'] }, 'ring': { name: 'Ring', icon: '💍', stats: ['pAtk', 'mAtk', 'luck', 'crit'] }, 'amulet': { name: 'Amulet', icon: '🧿', stats: ['maxHp', 'luck', 'evasion'] } };
    const itemDef = itemTypes[slot]; let fallbackStats = {}; let statName = itemDef.stats[0]; fallbackStats[statName] = 5;
...
async function initGame() {
    try {
        ...
        // Try to load save file before setting default hero!
        loadGame();
