...
function processTimeEffects(unit) {
    if (!unit || !unit.activeEffects) return; let uiChanged = false;
        ...
        if (eff.duration !== undefined) {
            ...
            if (eff.tickTimer >= 1000) {
                ...
                if (hpDmgPct > 0) {
                    ...
                    if (unit === player) { player.currentHealth -= mutatorDmg; updatePlayerHealthBar(); if (player.currentHealth <= 0) triggerGameOver("Choked by Toxic Air"); } else { unit.hp -= mutatorDmg; animateHit(unit.id, mutatorDmg, false); }
                }
                if (eff.type === 'poison') {
                    let dmg = Math.max(1, Math.floor(maxH * 0.05));
                    if (unit === player) { player.currentHealth -= dmg; updatePlayerHealthBar(); spawnFloatingText('player-combat-area', "POISON -" + dmg, "float-enemy-dmg"); if (player.currentHealth <= 0) triggerGameOver("Succumbed to poison"); } else { unit.hp -= dmg; animateHit(unit.id, dmg, false); }
                } 
                else if (eff.type === 'burn') {
                    let dmg = Math.max(1, Math.floor(maxH * 0.08));
                    if (unit === player) { player.currentHealth -= dmg; updatePlayerHealthBar(); spawnFloatingText('player-combat-area', "BURN -" + dmg, "float-enemy-dmg"); if (player.currentHealth <= 0) triggerGameOver("Burnt to a crisp"); } else { unit.hp -= dmg; animateHit(unit.id, dmg, false); }
                }
                else if (eff.type === 'regen') { applyHeal(unit, Math.max(1, Math.floor(maxH * 0.05))); }
            }
            if (eff.duration <= 0) {
                if (eff.type === 'doom') {
                    if (unit === player) { player.currentHealth = 0; updatePlayerHealthBar(); spawnFloatingText('player-combat-area', "DOOMED!", "float-enemy-dmg"); triggerGameOver("Erased from existence by Doom"); } else { unit.hp = 0; animateHit(unit.id, 9999, false); spawnFloatingText(`enemy-${unit.id}`, "DOOMED!", "float-crit"); }
                }
