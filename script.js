...
let player = {
    ...
    heroStats: {}, heroSkillLevels: {},
    equipment: { head: null, body: null, legs: null, boots: null, weapon: null, leftHand: null, ring: null, amulet: null },
    inventory: [], materials: {}, attackProgress: 0, activeEffects: [], highestStageUnlocked: 0 
};
...
let runStats = {
    ...
    runes: 0, expGained: 0, goldGained: 0, gemsGained: 0, enemiesKilled: 0, materialsGained: {}, 
    upgradeLevels: { p_atk: 0, m_atk: 0, spd: 0, splash: 0, double: 0, crit: 0, lifesteal: 0, evasion: 0, p_def: 0, m_def: 0 },
    hasRareUpgrade: false, hasUltimateUpgrade: false, commonUpgradeCounts: { heal: 0, gold: 0, temp_atk: 0, vitality: 0 }, maxHpBonus: 0
};
...
let activeEnemies = [];
let waveManager = { 
    wave: 1, currentBiomeIndex: 0, currentSubstageIndex: 0, 
    isUpgrading: false, transitioning: false, 
    normalEmojis: ['👾', '🧟', '🦇', '💀', '🕷️', '🦂'], bossEmojis: ['🐉', '👹', '🦑', '🦖'] 
};
...
// --- STAGE DEFINITIONS WITH DYNAMIC BACKGROUNDS ---
const STAGE_DATA = [
    { 
        id: 'forest', 
        name: 'Forest', 
    ...
    { 
        ...
        backgrounds: ['grave-1.jpg', 'grave-2.jpg', 'grave-3.jpg', 'grave-4.jpg', 'grave-5.jpg', 'grave-6.jpg'] 
    },
    { 
        id: 'ruins', 
        name: 'Ancient Ruins', 
        backgrounds: ['ruins-1.jpg', 'ruins-2.jpg', 'ruins-3.jpg', 'ruins-4.jpg', 'ruins-5.jpg', 'ruins-6.jpg'] 
    },
    { 
        id: 'coast', 
        name: 'Forbidden Coast', 
        backgrounds: ['coast-1.jpg', 'coast-2.jpg', 'coast-3.jpg', 'coast-4.jpg', 'coast-5.jpg', 'coast-6.jpg'] 
    },
    { 
        id: 'volcano', 
        name: 'Volcanic Crag', 
        backgrounds: ['volcano-1.jpg', 'volcano-2.jpg', 'volcano-3.jpg', 'volcano-4.jpg', 'volcano-5.jpg', 'volcano-6.jpg'] 
    },
    { 
        id: 'tundra', 
        name: 'Frozen Tundra', 
        backgrounds: ['tundra-1.jpg', 'tundra-2.jpg', 'tundra-3.jpg', 'tundra-4.jpg', 'tundra-5.jpg', 'tundra-6.jpg'] 
    },
    { 
        id: 'desert', 
        name: 'Scorched Desert', 
        backgrounds: ['desert-1.jpg', 'desert-2.jpg', 'desert-3.jpg', 'desert-4.jpg', 'desert-5.jpg', 'desert-6.jpg'] 
    },
    { 
        id: 'void', 
        name: 'Shadow Realm', 
        backgrounds: ['void-1.jpg', 'void-2.jpg', 'void-3.jpg', 'void-4.jpg', 'void-5.jpg', 'void-6.jpg'] 
    },
    { 
        id: 'celestial', 
        name: "Celestial Palace", 
        backgrounds: ['celestial-1.jpg', 'celestial-2.jpg', 'celestial-3.jpg', 'celestial-4.jpg', 'celestial-5.jpg', 'celestial-6.jpg'] 
    }
];
...
// --- SAVE & LOAD SYSTEM ---
function saveGame() {
    const saveData = { player: player }; 
    localStorage.setItem('pocketHeroesSave', JSON.stringify(saveData)); 
    showNotification("💾 Game Saved Successfully!");
}

function loadGame() {
    const savedData = localStorage.getItem('pocketHeroesSave');
    if (savedData) {
        try {
            ...
            showNotification("⚠️ Error loading save!"); 
        }
    } else { 
...
function wipeSaveData() {
    if (confirm("Are you sure? This will completely erase your heroes, forged gear, and region progress!")) {
        localStorage.removeItem('pocketHeroesSave'); 
        showNotification("⚠️ Data Wiped. Reloading..."); 
...
function renderStagesMenu() {
    ...
    STAGE_DATA.forEach((stage, index) => {
        ...
        if (player.highestStageUnlocked >= (index * 6) + 6) {
            progressInsideBiome = 6; 
        } else if (player.highestStageUnlocked >= index * 6) {
            progressInsideBiome = player.highestStageUnlocked - (index * 6);
        }
        ...
        container.innerHTML += `
            <div class="card ${stateClass}" ${clickAction}>
                ...
                <div class="card-info">
                    ...
                    <p style="color: ${isUnlocked ? '#f1c40f' : '#7f8c8d'}; font-size: 0.8rem;">Progress: ${progressInsideBiome}/6 Cleared</p>
                </div>
            </div>`;
    });
}
...
function renderHeroDetails(heroId) {
    ...
    contentDiv.innerHTML = `
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 5px; font-size: 0.9rem; color: #fff;">
        ...
        <p style="margin-top: 5px; color: #fff;"><b>Innate Passive:</b> <span style="color: #f1c40f;">${hero.innateDesc || 'None'}</span></p>
        <p style="margin-top: 5px; color: #fff;"><b>Active Skill:</b> <span style="color: #3498db;">${hero.innateSkill ? `(${skillChance}%) ${hero.innateSkill.desc}` : 'None'}</span></p>
        <button class="action-btn" style="width: 100%; margin-top: 15px; padding: 10px; background: #e67e22; box-shadow: 0 4px 0 #d35400; font-size: 1.1rem;" onclick="upgradeHeroSkill('${heroId}')">${btnText}</button>`;
    ...
    if (player.currentHero === heroId) { 
        ...
        btnActive.style.boxShadow = "0 6px 0 #27ae60";
        ...
        btnActive.style.boxShadow = "0 6px 0 #d35400";
        ...
        for(let i=0; i < totalSlots; i++) { 
            ...
            if (item) { 
                ...
                <div class="inv-slot" style="cursor:pointer; position: relative;" onclick="showGearModal(player.inventory[${i}], 'inventory', ${i})">${item.icon}
                    <div style="position: absolute; bottom: 2px; right: 2px; font-size: 0.5rem; background: rgba(0,0,0,0.6); color: #fff; padding: 1px 3px; border-radius: 3px;">${item.type}</div>
                </div>`; 
            } else { 
...
function showGearModal(item, source, key) {
    ...
    for (let stat in statNames) {
        ...
        if (itemVal > 0 || equippedVal > 0) {
            ...
            if (source === 'inventory') { 
                if (diff > 0) diffHtml = `<span class="stat-positive" style="color:#2ecc71;">(+${diff})</span>`; 
                else if (diff < 0) diffHtml = `<span class="stat-negative" style="color:#e74c3c;">(${diff})</span>`; 
                else diffHtml = `<span class="stat-neutral" style="color:#bdc3c7;">(-)</span>`; 
            }
    ...
    if (source === 'inventory') { 
        ...
        actionBtn.onclick = () => { equipItem(key, item.slot); document.getElementById('gear-details-modal').style.display = 'none'; document.getElementById('global-modal-backdrop').style.display='none';}; 
    } else { 
        actionBtn.innerText = 'UNEQUIP'; 
        actionBtn.style.background = '#e74c3c';
        actionBtn.onclick = () => { unequipItem(key); document.getElementById('gear-details-modal').style.display = 'none'; document.getElementById('global-modal-backdrop').style.display='none';}; 
    }
...
function updateGearStatsPanel() {
    ...
    html += `<div class="equip-stat-row"><span>Max HP</span> <span style="color:#fff;">${maxH}</span></div>`;
    html += `<div class="equip-stat-row"><span style="color:${hero.attackType === 'magical' ? '#7f8c8d' : '#bdc3c7'}">P.Atk</span> <span style="color:${hero.attackType === 'magical' ? '#7f8c8d' : '#fff'}">${stats.pAtk}</span></div>`;
    html += `<div class="equip-stat-row"><span style="color:${hero.attackType === 'physical' ? '#7f8c8d' : '#bdc3c7'}">M.Atk</span> <span style="color:${hero.attackType === 'physical' ? '#7f8c8d' : '#fff'}">${stats.mAtk}</span></div>`;
    html += `<div class="equip-stat-row"><span>P.Def</span> <span style="color:#fff;">${stats.pDef}</span></div>`;
    html += `<div class="equip-stat-row"><span>M.Def</span> <span style="color:#fff;">${stats.mDef}</span></div>`;
    html += `<div class="equip-stat-row"><span>Atk Spd</span> <span style="color:#fff;">${stats.atkSpd.toFixed(2)}</span></div>`;
    html += `<div class="equip-stat-row"><span>Speed</span> <span style="color:#fff;">${stats.spd}</span></div>`;
    html += `<div class="equip-stat-row"><span>Crit %</span> <span style="color:#fff;">${(stats.crit * 100).toFixed(1)}%</span></div>`;
    html += `<div class="equip-stat-row"><span>Evasion %</span> <span style="color:#fff;">${(stats.evasion * 100).toFixed(1)}%</span></div>`;
    html += `<div class="equip-stat-row"><span>Luck</span> <span style="color:#fff;">${stats.luck}</span></div>`; 
...
function switchShopTab(tab) {
    ...
    let btnForge = document.getElementById('tab-forge');
    let btnPremium = document.getElementById('tab-premium');
...
function renderForge() {
    ...
    for(let matId in player.materials) { 
        if(player.materials[matId] > 0) { 
            ...
            matsContainer.innerHTML += `<div class="currency" title="${mData.name}" style="background: rgba(0,0,0,0.5); border: 1px solid rgba(255,255,255,0.1); border-radius: 5px; padding: 4px 8px; color: #fff;">${mData.icon} ${player.materials[matId]}</div>`; 
        } 
    }
    ...
    recipesData.forEach(recipe => {
        ...
        if (player.gold < recipe.goldCost) canCraft = false; 
        
        costHtml += `<span style="color: ${player.gold >= recipe.goldCost ? '#f1c40f' : '#e74c3c'}">🪙 ${recipe.goldCost}</span>`;
        for(let matId in recipe.materials) {
            let needed = recipe.materials[matId]; 
            let owned = player.materials[matId] || 0; 
            if (owned < needed) canCraft = false; 
            let mData = materialsData[matId] || { icon: '📦' };
            costHtml += `<span style="color: ${owned >= needed ? '#2ecc71' : '#e74c3c'}">${mData.icon} ${owned}/${needed}</span>`;
        }
        costHtml += `</div>`;
        
        container.innerHTML += `
        <div class="card ${canCraft ? '' : 'locked'}" ${canCraft ? `onclick="craftItem('${recipe.id}')"` : ''}>
            <div class="card-icon" style="font-size: 2.5rem;">${recipe.icon}</div>
            <div class="card-info">
                <h3 style="color: #fff; margin:0; font-size:1rem;">${recipe.name} <span style="font-size:0.7rem; color:#bdc3c7;">(${recipe.slot})</span></h3>
                <p style="color: #bdc3c7; font-size: 0.8rem; margin: 2px 0;">${recipe.desc}</p>
                ${costHtml}
            </div>
        </div>`;
    });
...
function buyPremium(item) {
    ...
    else if (item === 'chest' && player.gems >= 20) { player.gems -= 20; let newGear = generateRandomEquipment(); player.inventory.push(newGear); updateUI(); showNotification(`You got a ${newGear.name}!`); } 
...
function dropMaterial(matId, amount, unitId) {
    ...
    if (!runStats.materialsGained) runStats.materialsGained = {}; 
...
function renderMaterialRecap(containerId) {
    ...
    for (let matId in runStats.materialsGained) { 
        ...
        if (amount > 0) { 
            hasMats = true; 
            let mData = materialsData[matId] || { icon: '📦', name: 'Unknown' }; 
            container.innerHTML += `<div class="currency" title="${mData.name}" style="background: rgba(0,0,0,0.5); border-radius: 5px; padding: 4px 8px; color: #fff;">${mData.icon} ${amount}</div>`; 
        } 
    }
...
function applyHeal(unit, amount) {
    if (hasStatus(unit, 'decay')) {
        ...
        if (unit === player) { 
            ...
            if (hpBarDiv) hpBarDiv.style.width = (unit.hp / unit.maxHp) * 100 + '%'; if (hpTextDiv) hpTextDiv.innerText = `${Math.ceil(unit.hp)}/${unit.maxHp}`; spawnFloatingText(`enemy-${unit.id}`, "+" + amount, "float-heal"); 
        }
    }
}
...
function processTimeEffects(unit) {
    ...
    let uiChanged = false; let finalizeKill = false;
    ...
    for (let i = unit.activeEffects.length - 1; i >= 0; i--) {
        ...
        if (eff.duration !== undefined) {
            ...
            if (eff.tickTimer >= 1000) {
                eff.tickTimer = 0; let maxH = unit === player ? getMaxHealth() : unit.maxHp; let hpDmgPct = getMutatorMod('hpDmgPerSec', 0);
                if (hpDmgPct > 0) { let mutatorDmg = Math.max(1, Math.floor(maxH * hpDmgPct)); if (unit === player) { player.currentHealth -= mutatorDmg; if (player.currentHealth <= 0) triggerGameOver("Choked by Toxic Air"); } else { unit.hp -= mutatorDmg; animateHit(unit.id, mutatorDmg, false); } }
                if (eff.type === 'poison') { let dmg = Math.max(1, Math.floor(maxH * 0.05)); if (unit === player) { player.currentHealth -= dmg; spawnFloatingText('player-combat-area', "POISON -" + dmg, "float-enemy-dmg"); if (player.currentHealth <= 0) triggerGameOver("Succumbed to poison"); } else { unit.hp -= dmg; animateHit(unit.id, dmg, false); } } 
                else if (eff.type === 'burn') { let dmg = Math.max(1, Math.floor(maxH * 0.08)); if (unit === player) { player.currentHealth -= dmg; spawnFloatingText('player-combat-area', "BURN -" + dmg, "float-enemy-dmg"); if (player.currentHealth <= 0) triggerGameOver("Burnt to a crisp"); } else { unit.hp -= dmg; animateHit(unit.id, dmg, false); } }
                else if (eff.type === 'regen') { applyHeal(unit, Math.max(1, Math.floor(maxH * 0.05))); }
            }
            if (eff.duration <= 0) {
                if (eff.type === 'doom') { if (unit === player) { player.currentHealth = 0; spawnFloatingText('player-combat-area', "DOOMED!", "float-enemy-dmg"); triggerGameOver("Erased from existence by Doom"); } else { unit.hp = 0; finalizeKill = true; animateHit(unit.id, 9999, false); spawnFloatingText(`enemy-${unit.id}`, "DOOMED!", "float-crit"); } }
    ...
    if (newHp <= 0 && unit !== player && !unit.isDead) { finalizeKill = true; }
    
    if (finalizeKill) { 
        unit.activeEffects = []; 
        let sCont = document.getElementById(`enemy-status-${unit.id}`); if (sCont) sCont.innerHTML = ''; 
        let atbBar = document.getElementById(`enemy-atb-bar-${unit.id}`); if (atbBar) atbBar.style.width = '0%'; 
        let unitDiv = document.getElementById(`enemy-${unit.id}`); handleEnemyDeath(unit, unit.id, unitDiv); 
    } else {
        if (unit === player) updatePlayerHealthBar(); 
...
function createStatusHtml(eff, mini = false) {
    ...
    if (eff.type === 'stun') { text = "Stun"; iconClass = "debuff"; emoji = "💫"; } else if (eff.type === 'poison') { text = "Poison"; iconClass = "debuff"; emoji = "☠️"; } else if (eff.type === 'burn') { text = "Burn"; iconClass = "debuff"; emoji = "🔥"; } else if (eff.type === 'bleed') { text = "Bleed"; iconClass = "debuff"; emoji = "🩸"; } else if (eff.type === 'decay') { text = "Decay"; iconClass = "debuff"; emoji = "🧟"; } else if (eff.type === 'slow') { text = "Slow"; iconClass = "debuff"; emoji = "🐢"; } else if (eff.type === 'haste') { text = "Haste"; iconClass = "buff"; emoji = "⚡"; } else if (eff.type === 'blind') { text = "Blind"; iconClass = "debuff"; emoji = "👁️‍🗨️"; } else if (eff.type === 'empower') { text = "Empower"; iconClass = "buff"; emoji = "🔥"; } else if (eff.type === 'vulnerable') { text = "Vuln"; iconClass = "debuff"; emoji = "💔"; } else if (eff.type === 'barrier') { text = "Barrier"; iconClass = "buff"; emoji = "🛡️"; } else if (eff.type === 'regen') { text = "Regen"; iconClass = "buff"; emoji = "💖"; } else if (eff.type === 'thorns') { text = "Thorns"; iconClass = "buff"; emoji = "🌵"; } else if (eff.type === 'berserk') { text = "Berserk"; iconClass = "buff"; emoji = "😡"; } else if (eff.type === 'block') { text = "Block"; iconClass = "buff"; emoji = "🛡️"; } else if (eff.type === 'focused') { text = "Focus"; iconClass = "buff"; emoji = "👁️"; } else if (eff.type === 'marked') { text = "Marked"; iconClass = "debuff"; emoji = "🎯"; } else if (eff.type === 'doom') { text = "Doom"; iconClass = "debuff"; emoji = "⏳"; }
    ...
    return `<div class="status-icon ${iconClass}" style="background: rgba(0,0,0,0.6); padding: 2px 6px; border-radius: 5px; margin: 2px;">${emoji} ${text} (${durText})</div>`;
}
...
function renderStatusEffects() {
    ...
    if (pCont) { 
        pCont.innerHTML = ''; 
        if (activeEnemies && activeEnemies.find(e => e.hp > 0 && enemySkillsData[e.skill] && enemySkillsData[e.skill].aura === 'intimidate')) { pCont.innerHTML += `<div class="status-icon debuff" style="background: rgba(0,0,0,0.6); padding: 2px 6px; border-radius: 5px; margin: 2px;" title="Intimidated">💀 Intimidated</div>`; } 
    ...
    activeEnemies.forEach(e => { 
        ...
        if (eCont) { eCont.innerHTML = ''; if (e.activeEffects) { e.activeEffects.forEach(eff => { eCont.innerHTML += createStatusHtml(eff, true); }); } } 
    });
}
...
function debugMutator(type) {
    ...
    if (type === 'clear') { activeMutator = null; if(mutatorEl) mutatorEl.style.display = 'none'; showNotification("Mutators Cleared"); } 
    else if (type === 'magic_immune') { activeMutator = { name: "Anti-Magic Field", modifiers: { nullifyMagic: true } }; if(mutatorEl) { mutatorEl.style.display = 'block'; mutatorEl.innerText = "MUTATOR: Anti-Magic Field"; } showNotification("Magic Damage Nullified!"); } 
    else if (type === 'toxic') { activeMutator = { name: "Toxic Air", modifiers: { hpDmgPerSec: 0.02 } }; if(mutatorEl) { mutatorEl.style.display = 'block'; mutatorEl.innerText = "MUTATOR: Toxic Air (-2% HP/sec)"; } showNotification("Toxic Air Mutator Applied!"); }
    updateCombatStatsPanel();
}
...
function playBattleStartAnimation() { 
    ...
    if(overlay && swords) { 
        overlay.style.display = 'flex'; swords.style.transform = 'scale(0.1)'; swords.style.opacity = '0';
        setTimeout(() => { swords.style.transition = 'all 0.5s ease-out'; swords.style.transform = 'scale(1.5)'; swords.style.opacity = '1'; }, 50);
...
function spawnFloatingText(targetId, text, className) { 
    ...
    ft.style.position = 'absolute'; ft.style.fontWeight = 'bold'; ft.style.zIndex = '100'; ft.style.textShadow = '0 2px 4px #000';
    if(className === 'float-dmg') { ft.style.color = '#fff'; ft.style.fontSize = '1.2rem'; }
    if(className === 'float-crit') { ft.style.color = '#f1c40f'; ft.style.fontSize = '1.5rem'; }
    if(className === 'float-heal') { ft.style.color = '#2ecc71'; ft.style.fontSize = '1.2rem'; }
    if(className === 'float-miss') { ft.style.color = '#bdc3c7'; ft.style.fontSize = '1rem'; }
    if(className === 'float-enemy-dmg') { ft.style.color = '#e74c3c'; ft.style.fontSize = '1.2rem'; }
    
    ft.innerText = text; 
    let offsetX = (Math.random() * 40) - 20; 
    ft.style.left = `calc(50% + ${offsetX}px)`; ft.style.top = '10px'; 
    ft.style.transition = 'all 1s ease-out';
    targetDiv.appendChild(ft); 
    setTimeout(() => { ft.style.transform = 'translateY(-40px)'; ft.style.opacity = '0'; }, 50);
    setTimeout(() => { if(ft.parentElement) ft.remove(); }, 1050); 
}
...
function spawnLootDrop(targetId, type) { 
    ...
    let ft = document.createElement('div'); 
    ft.innerText = emoji; 
    ft.style.position = 'absolute'; ft.style.fontSize = '1.5rem'; ft.style.zIndex = '90'; ft.style.filter = 'drop-shadow(0 2px 2px #000)';
    let dirX = (Math.random() > 0.5 ? 1 : -1) * (0.5 + Math.random()); 
    ft.style.left = '50%'; ft.style.top = '30%'; 
    ft.style.transition = 'all 0.8s cubic-bezier(.25,.8,.25,1)';
    targetDiv.appendChild(ft); 
    setTimeout(() => { ft.style.transform = `translate(${dirX * 60}px, -40px) scale(1.5)`; }, 50);
    setTimeout(() => { ft.style.transform = `translate(${dirX * 80}px, 20px) scale(1)`; ft.style.opacity = '0'; }, 400);
    setTimeout(() => { if(ft.parentElement) ft.remove(); }, 850); 
}
...
function combatTick() {
    ...
    aliveEnemies.forEach(e => {
        if (!hasStatus(e, 'stun')) { let eAtkSpd = e.atkSpd || 1.0; if (hasStatus(e, 'haste')) eAtkSpd *= 1.5; if (hasStatus(e, 'slow')) eAtkSpd *= 0.5; e.attackProgress += (eAtkSpd * gameSpeed * 2.5 * atbMult); if (e.attackProgress >= 100) { e.attackProgress = 100; actionQueue.push({ type: 'enemy', entity: e, spd: e.spd }); } }
    ...
    if (actionQueue.length > 0) {
        ...
        actionQueue.forEach(action => {
            if (action.type === 'player' && player.attackProgress >= 100 && player.currentHealth > 0) { executePlayerAttack(); player.attackProgress = 0; if(pAtb) pAtb.style.width = '0%'; } 
            else if (action.type === 'enemy' && action.entity.hp > 0 && action.entity.attackProgress >= 100 && player.currentHealth > 0) { executeEnemyAttack(action.entity); action.entity.attackProgress = 0; let atbBar = document.getElementById(`enemy-atb-bar-${action.entity.id}`); if (atbBar) atbBar.style.width = '0%'; }
        });
    }
}
...
function startTestBattle() {
    ...
    updateCombatStatsPanel(); updatePlayerHealthBar(); openMenu('game');

    let container = document.getElementById('enemy-container'); let textEl = document.getElementById('level-wave-text'); let mutatorEl = document.getElementById('mutator-display'); if(mutatorEl) mutatorEl.style.display = 'none'; activeMutator = null; container.innerHTML = ''; activeEnemies = []; selectedTargetId = null;
    ...
    let dummyHp = 999999; let dummy = { id: 0, maxHp: dummyHp, hp: dummyHp, pAtk: 0, mAtk: 0, pDef: 9999, mDef: 9999, exp: 0, skill: 'dummy', attackProgress: 0, activeEffects: [], isDead: false, isBoss: true, spd: 1, atkSpd: 0.1, name: "Training Dummy" }; activeEnemies.push(dummy);
    container.innerHTML = `<div class="enemy-unit boss" id="enemy-0" onclick="selectTarget(0)" style="display:flex; flex-direction:column; align-items:center; cursor:pointer;"><div class="emoji" style="font-size: 5rem;">🎯</div><div class="enemy-name" style="font-size: 1.2rem; font-weight: bold; margin-bottom: 4px; color:#fff; text-shadow:0 1px 2px #000;">Training Dummy</div><div class="mini-bar-container" style="width:60px; height:8px; background:rgba(0,0,0,0.5);"><div class="mini-bar-fill" id="enemy-hp-bar-0" style="height:100%; background:#e74c3c;"></div></div><div class="mini-bar-container" style="width:60px; height: 6px; margin-top: 2px; background:rgba(0,0,0,0.5);"><div class="mini-bar-fill" id="enemy-atb-bar-0" style="background: #f1c40f; width: 0%; transition: none; height:100%;"></div></div><div class="mini-hp-text" id="enemy-hp-text-0" style="font-size:0.8rem; color:#fff; font-weight:bold;">${dummyHp}/${dummyHp}</div><div id="enemy-status-0" style="display:flex; justify-content:center; gap:2px; margin-top:2px; height:15px; color: white;"></div><div class="boss-skill-badge" style="background:#34495e; color:#fff; padding:2px 8px; border-radius:10px; font-size:0.7rem; border:1px solid #7f8c8d; margin-top:5px;">🛡️ 9999 DEF/MDEF</div></div>`;
    player.attackProgress = 95; dummy.attackProgress = 0; let pAtb = document.getElementById('player-atb'); if(pAtb) pAtb.style.width = '95%';
    renderStatusEffects(); playBattleStartAnimation(); setTimeout(() => { waveManager.isUpgrading = false; startCombatLoop(); }, 1500); 
}
...
function startGame() {
    ...
    waveManager.isUpgrading = true; waveManager.transitioning = false; 
    updateCombatStatsPanel(); updatePlayerHealthBar(); openMenu('game'); spawnEnemyPack(); playBattleStartAnimation(); 
...
function renderBossUI(id, name, emoji, hp, bSkill) {
    ...
    container.innerHTML += `<div class="enemy-unit boss" id="enemy-${id}" onclick="selectTarget(${id})" style="display:flex; flex-direction:column; align-items:center; cursor:pointer;"><div class="emoji" style="font-size: 5rem; filter:drop-shadow(0 5px 5px rgba(0,0,0,0.5));">${emoji}</div><div class="enemy-name" style="font-size: 1.2rem; font-weight: bold; margin-bottom: 4px; color:#fff; text-shadow:0 1px 2px #000;">${name}</div><div class="mini-bar-container" style="width:60px; height:8px; background:rgba(0,0,0,0.5);"><div class="mini-bar-fill" id="enemy-hp-bar-${id}" style="height:100%; background:#e74c3c;"></div></div><div class="mini-bar-container" style="width:60px; height: 6px; margin-top: 2px; background:rgba(0,0,0,0.5);"><div class="mini-bar-fill" id="enemy-atb-bar-${id}" style="background: #f1c40f; width: 0%; transition: none; height:100%;"></div></div><div class="mini-hp-text" id="enemy-hp-text-${id}" style="font-size:0.8rem; color:#fff; font-weight:bold;">${hp}/${hp}</div><div id="enemy-status-${id}" style="display:flex; justify-content:center; gap:2px; margin-top:2px; height:15px; color: white;"></div>${bSkill && bSkill.name ? `<div class="boss-skill-badge" style="background:#34495e; color:#fff; padding:2px 8px; border-radius:10px; font-size:0.7rem; border:1px solid #7f8c8d; margin-top:5px;">${bSkill.icon || '🔥'} ${bSkill.name}</div>` : ''}</div>`;
}

function renderNormalEnemyUI(id, enemyData, emoji, isElite) {
    let container = document.getElementById('enemy-container'); 
    let eliteStyle = isElite ? 'filter: drop-shadow(0 0 10px #f1c40f);' : 'filter: drop-shadow(0 5px 5px rgba(0,0,0,0.5));';
    container.innerHTML += `<div class="enemy-unit" id="enemy-${id}" onclick="selectTarget(${id})" style="display:flex; flex-direction:column; align-items:center; cursor:pointer;"><div class="emoji" style="font-size: 3.5rem; ${eliteStyle}">${emoji}</div><div class="enemy-name" style="font-size: 0.8rem; font-weight: bold; margin-bottom: 2px; color:#fff; text-shadow:0 1px 2px #000;">${enemyData.name}</div><div class="mini-bar-container" style="width:50px; height:6px; background:rgba(0,0,0,0.5);"><div class="mini-bar-fill" id="enemy-hp-bar-${id}" style="height:100%; background:#e74c3c;"></div></div><div class="mini-bar-container" style="width:50px; height: 4px; margin-top: 2px; background:rgba(0,0,0,0.5);"><div class="mini-bar-fill" id="enemy-atb-bar-${id}" style="background: #f1c40f; width: 0%; transition: none; height:100%;"></div></div><div class="mini-hp-text" id="enemy-hp-text-${id}" style="font-size:0.7rem; color:#fff; font-weight:bold;">${enemyData.hp}/${enemyData.hp}</div><div id="enemy-status-${id}" style="display:flex; justify-content:center; gap:2px; margin-top:2px; height:15px; color: white;"></div></div>`;
}
...
function spawnEnemyPack() {
    ...
    if (stageInfo.isBiomeBoss) {
        ...
        let nodeName = SUBSTAGE_NAMES[stageInfo.substageIndex]; textEl.innerHTML = `<span style="font-size: 1rem; color: #bdc3c7;">[${stageInfo.biome.name.toUpperCase()}]</span><br>Stage ${stageInfo.biomeIndex + 1}-${stageInfo.substageIndex + 1} (${nodeName})<br><span style="font-size: 0.8rem">Wave ${stageInfo.wave}</span>`;
        let enemyCount = 1; let spawnList = []; let guaranteedElites = 0;
        switch(stageInfo.wave) {
            case 1: enemyCount = Math.floor(Math.random() * 2) + 1; spawnList = Array.from({length: enemyCount}, () => tier1[Math.floor(Math.random() * tier1.length)]); break;
            case 2: enemyCount = Math.floor(Math.random() * 2) + 2; spawnList = Array.from({length: enemyCount}, () => tier1[Math.floor(Math.random() * tier1.length)]); break;
            case 3: enemyCount = 3; let pool3 = [...tier1, ...tier2]; spawnList = Array.from({length: enemyCount}, () => pool3[Math.floor(Math.random() * pool3.length)]); break;
            case 4: enemyCount = isOutskirts ? 3 : Math.floor(Math.random() * 2) + 3; let pool4 = [...tier1, ...tier2]; spawnList = Array.from({length: enemyCount}, () => pool4[Math.floor(Math.random() * pool4.length)]); if (!isOutskirts) guaranteedElites = 1; break;
            case 6: enemyCount = isOutskirts ? 3 : Math.floor(Math.random() * 2) + 3; let pool6 = isGauntlet ? tier2 : [...tier1, ...tier2]; spawnList = Array.from({length: enemyCount}, () => pool6[Math.floor(Math.random() * pool6.length)]); break;
            case 7: enemyCount = 3; let pool7 = isGauntlet ? [...tier2, ...tier3] : [...tier2, ...tier3]; spawnList = Array.from({length: enemyCount}, () => pool7[Math.floor(Math.random() * pool7.length)]); break;
            case 8: enemyCount = isOutskirts ? 3 : Math.floor(Math.random() * 2) + 4; let pool8 = isGauntlet ? [...tier2, ...tier3] : [...tier1, ...tier2, ...tier3]; spawnList = Array.from({length: enemyCount}, () => pool8[Math.floor(Math.random() * pool8.length)]); if (!isOutskirts) guaranteedElites = 1; break;
            case 9: enemyCount = isOutskirts ? 3 : 5; let pool9 = isGauntlet ? [...tier2, ...tier3] : [...tier1, ...tier2, ...tier3]; spawnList = Array.from({length: enemyCount}, () => pool9[Math.floor(Math.random() * pool9.length)]); if (!isOutskirts) guaranteedElites = 2; break;
            default: spawnList = [tier1[0]]; break;
        }
    ...
    let pStats = getPlayerStats(); let combatants = []; combatants.push({ type: 'player', spd: pStats.spd, atkSpd: Math.max(0.1, pStats.atkSpd), ref: player });
    activeEnemies.forEach(e => { e.spd = e.spd || (e.isBoss ? 15 : (e.isElite ? 12 : 10)); e.atkSpd = e.atkSpd || (e.isBoss ? 1.2 : 1.0); combatants.push({ type: 'enemy', spd: e.spd, atkSpd: e.atkSpd, ref: e }); });
    combatants.sort((a, b) => b.spd - a.spd);
    combatants.forEach((c, index) => {
        let progressPerTick = c.atkSpd * gameSpeed * 2.5; let staggerTicks = index * 10; let initialProgress = 100 - (staggerTicks * progressPerTick);
...
function handleEnemyDeath(target, unitId, unitDiv) {
    ...
    if (skillData && skillData.trigger === 'death' && !target.hasRevived) {
        if (skillData.reviveFraction) {
            target.hasRevived = true; target.hp = Math.floor(target.maxHp * skillData.reviveFraction); 
            let hpBarDiv = document.getElementById(`enemy-hp-bar-${unitId}`); let hpTextDiv = document.getElementById(`enemy-hp-text-${unitId}`);
            if (hpBarDiv) hpBarDiv.style.width = (target.hp / target.maxHp) * 100 + '%'; if (hpTextDiv) hpTextDiv.innerText = `${target.hp}/${target.maxHp}`;
            spawnFloatingText(`enemy-${unitId}`, skillData.floatingText || "REVIVED!", "float-heal"); return; 
        }
    }
    ...
    if (unitId === selectedTargetId) selectedTargetId = null;

    if(unitDiv) { 
        unitDiv.style.transition = 'all 0.3s ease-out';
        unitDiv.style.opacity = '0';
        unitDiv.style.transform = 'scale(0.5)';
        setTimeout(() => unitDiv.style.display = 'none', 300); 
    }

    try {
        let expReward = target.exp || 10; runStats.expGained += expReward;
        let lootTable = target.loot || {};
        ...
        if (target.isBoss) {
            ...
            let stats = getPlayerStats(); let goldEarned = Math.floor(10 * (1 + (stats.luck || 0))); 
            ...
            if (lootTable.very_rare && Math.random() < 0.03) dropMaterial(lootTable.very_rare, 1, unitId);
            if (lootTable.super_rare && Math.random() < 0.01) dropMaterial(lootTable.super_rare, 1, unitId);
        ...
        document.getElementById('run-runes-text').innerText = runStats.runes;
    } catch (e) { console.error("Loot system failed: ", e); }

    if (activeEnemies.length > 0 && activeEnemies.every(e => e.isDead)) { if (!waveManager.transitioning) { waveManager.transitioning = true; setTimeout(() => packDefeated(), 400); } }
}
...
function animateHit(unitId, damageDealt, isCrit) {
    ...
    if (unitDiv) { 
        unitDiv.style.transition = 'none';
        unitDiv.style.transform = 'translateX(-5px)';
        unitDiv.style.filter = isCrit ? "drop-shadow(0 0 10px red)" : "brightness(1.5) sepia(1)";
        
        if(isCrit) { spawnFloatingText(`enemy-${unitId}`, `${damageDealt} CRIT!`, "float-crit"); } 
        else { spawnFloatingText(`enemy-${unitId}`, damageDealt, "float-dmg"); } 
        
        setTimeout(() => { 
            unitDiv.style.transition = 'all 0.2s';
            unitDiv.style.transform = 'translateX(0px)';
            unitDiv.style.filter = ""; 
        }, 100); 
    }
    let displayHp = Math.max(0, Math.ceil(target.hp));
    if (hpBarDiv) hpBarDiv.style.width = (displayHp / target.maxHp) * 100 + '%'; if (hpTextDiv) hpTextDiv.innerText = `${displayHp}/${target.maxHp}`;
...
function executePlayerAttack() {
    ...
    if (hasStatus(player, 'bleed')) { let bDmg = Math.floor(getMaxHealth() * 0.05); player.currentHealth -= bDmg; updatePlayerHealthBar(); spawnFloatingText('player-combat-area', "BLEED -" + bDmg, "float-enemy-dmg"); if (player.currentHealth <= 0) { triggerGameOver("Died from severe blood loss"); return; } }
    ...
    if(pIcon) { 
        pIcon.style.transition = 'none'; pIcon.style.transform = 'translateY(-20px) scale(1.1)';
        setTimeout(() => { pIcon.style.transition = 'all 0.2s'; pIcon.style.transform = 'translateY(0) scale(1)'; }, 100); 
    }
    ...
    for (let s = 0; s < strikes; s++) {
        setTimeout(() => {
            let target = null;
            if (selectedTargetId !== null) { target = activeEnemies.find(e => e.id === selectedTargetId && e.hp > 0 && !e.isDead); }
            if (!target) { target = activeEnemies.find(e => e.hp > 0 && !e.isDead); }
            if (!target) return;

            let damages = getTotalDamage(); let stats = getPlayerStats(); let isCrit = Math.random() < stats.crit;
            if (consumeCharge(player, 'focused')) isCrit = true; if (consumeCharge(target, 'marked')) isCrit = true;

            let eDefP = target.pDef || 0; let eDefM = target.mDef || 0;
            if (hasStatus(target, 'berserk')) { eDefP = 0; eDefM = 0; }

            let pDmg = calculateDamage(damages.pDmg, eDefP); let mDmg = calculateDamage(damages.mDmg, eDefM);
            ...
            if (defSkill && defSkill.trigger === 'defend') { if (defSkill.physicalDamageMultiplier !== undefined) pDmg = Math.floor(pDmg * defSkill.physicalDamageMultiplier); if (defSkill.magicDamageMultiplier !== undefined) mDmg = Math.floor(mDmg * defSkill.magicDamageMultiplier); }

            let dmg = pDmg + mDmg; if (isCrit) dmg = Math.floor(dmg * 2.5); if (isEmpowered) { dmg *= 2; spawnFloatingText('player-combat-area', "EMPOWERED!", "float-crit"); } if (hasStatus(player, 'berserk')) dmg *= 2; if (hasStatus(target, 'vulnerable')) dmg = Math.floor(dmg * 1.5); if (hasStatus(target, 'barrier')) dmg = Math.floor(dmg * 0.5);

            let hero = heroData[player.currentHero]; let innateLvl = player.heroSkillLevels[player.currentHero] || 0; let innateTrigger = false;
            ...
            if (innateTrigger) {
                ...
                else if (hero.innateSkill.type === 'warlock_curse') { dmg = dmg * 3; player.currentHealth -= Math.floor(getMaxHealth() * 0.05); updatePlayerHealthBar(); if (player.currentHealth <= 0) { triggerGameOver("Consumed by Dark Magic"); return; } }
            }
            ...
            if (consumeCharge(target, 'block')) { spawnFloatingText(`enemy-${target.id}`, "BLOCKED!", "float-miss"); dmg = 0; } 
                ...
                if (target.hp > 0 && target.hp <= target.maxHp * 0.5) { let tSkill = target.skill ? enemySkillsData[target.skill] : null; if (tSkill && tSkill.trigger === 'hp_threshold') { if (tSkill.enrageMultiplier && !target.hasEnraged) { target.hasEnraged = true; spawnFloatingText(`enemy-${target.id}`, tSkill.floatingText || "ENRAGED!", "float-enemy-dmg"); target.pAtk = Math.floor((target.pAtk || target.damage || 10) * tSkill.enrageMultiplier); target.mAtk = Math.floor((target.mAtk || target.damage || 10) * tSkill.enrageMultiplier); target.atkSpd = (target.atkSpd || 1) * tSkill.enrageMultiplier; } if (tSkill.summonEnemyId && !target.hasSummoned) { target.hasSummoned = true; spawnFloatingText(`enemy-${target.id}`, tSkill.floatingText || "SUMMON!", "float-enemy-dmg"); let count = tSkill.summonCount || 1; for(let i=0; i<count; i++) summonEnemyMidBattle(tSkill.summonEnemyId, tSkill.summonHpOverride); } } }
                if (player.equipment && player.equipment.weapon && player.equipment.weapon.onHit) { let hitEff = player.equipment.weapon.onHit; if (Math.random() < hitEff.chance) { applyStatus(target, hitEff.type, hitEff.duration); spawnFloatingText(`enemy-${target.id}`, `${hitEff.type.toUpperCase()}!`, "float-enemy-dmg"); } }
                if (defSkill && defSkill.trigger === 'defend' && defSkill.reflectPhysicalFraction && pDmg > 0) { let tDmg = Math.floor(pDmg * defSkill.reflectPhysicalFraction); player.currentHealth -= tDmg; updatePlayerHealthBar(); spawnFloatingText('player-combat-area', "REFLECT -" + tDmg, "float-enemy-dmg"); if (player.currentHealth <= 0) { triggerGameOver("Slain by " + target.name + "'s Thorns"); return; } }
                if (hasStatus(target, 'thorns') && dmg > 0) { let tDmg = Math.floor(dmg * 0.2); player.currentHealth -= tDmg; updatePlayerHealthBar(); spawnFloatingText('player-combat-area', "THORNS -" + tDmg, "float-enemy-dmg"); if (player.currentHealth <= 0) { triggerGameOver("Slain by " + target.name + "'s Thorns"); return; } }
            }
            ...
            if (innateTrigger) {
                ...
                else if (hero.innateSkill.type === 'paladin_heal') { applyHeal(player, Math.floor(getMaxHealth() * 0.20)); } 
                ...
                else if (hero.innateSkill.type === 'monk_combo') { target.hp -= dmg; animateHit(target.id, dmg, false); target.hp -= dmg; animateHit(target.id, dmg, false); } 
                else if (hero.innateSkill.type === 'bard_song') { runStats.runes += 1; spawnLootDrop('player-combat-area', 'rune'); spawnFloatingText('in-run-currency', '+1', 'float-rune'); } 
...
function executeEnemyAttack(e) {
    ...
    if(unitDiv) { 
        unitDiv.style.transition = 'none'; unitDiv.style.transform = 'translateY(15px) scale(1.1)';
        setTimeout(() => { unitDiv.style.transition = 'all 0.2s'; unitDiv.style.transform = 'translateY(0) scale(1)'; }, 150); 
    }

    let stats = getPlayerStats(); let incomingDmg = 0;
    let skillData = e.skill ? enemySkillsData[e.skill] : null; let isMagic = skillData && skillData.isMagic; let ignoresEvasion = skillData && skillData.ignoresEvasion; let isPiercing = skillData && skillData.isPiercing;
    let skillTriggered = false; if (skillData) { if (skillData.trigger === 'always') skillTriggered = true; if (skillData.trigger === 'attack' && skillData.chance && Math.random() < skillData.chance) skillTriggered = true; }
    let isCrit = false; if (skillTriggered && skillData.floatingText === 'CRIT!') isCrit = true; if (consumeCharge(e, 'focused')) isCrit = true; if (consumeCharge(player, 'marked')) isCrit = true;
    if (!isMagic && !isCrit && !ignoresEvasion && Math.random() < stats.evasion) { spawnFloatingText('player-combat-area', "MISS!", "float-miss"); return; }

    let skillDamageMultiplier = 1.0;
    if (skillTriggered) { if (skillData.damageMultiplier) skillDamageMultiplier = skillData.damageMultiplier; if (skillData.applyStatus) applyStatus(player, skillData.applyStatus.type, skillData.applyStatus.duration); if (skillData.floatingText && skillData.floatingText !== 'CRIT!') { spawnFloatingText('player-combat-area', skillData.floatingText, "float-enemy-dmg"); } }
    if (consumeCharge(player, 'block')) { spawnFloatingText('player-combat-area', "BLOCKED!", "float-miss"); return; }

    if (isMagic) {
        let rawDmg = e.mAtk || 0; if (runStats.purificationActive) { spawnFloatingText('player-combat-area', "IMMUNE!", "float-miss"); return; } if (getMutatorMod('nullifyMagic', false)) { spawnFloatingText(`enemy-${e.id}`, "NULLIFIED!", "float-miss"); return; }
        incomingDmg = isPiercing ? rawDmg : calculateDamage(rawDmg, stats.mDef);
    } else {
        let rawDmg = e.pAtk || 0; incomingDmg = isPiercing ? rawDmg : calculateDamage(rawDmg, stats.pDef);
    }
    ...
    if (isEmpowered) incomingDmg *= 2; if (hasStatus(e, 'berserk')) incomingDmg *= 2; if (hasStatus(player, 'vulnerable')) incomingDmg = Math.floor(incomingDmg * 1.5); if (hasStatus(player, 'barrier')) incomingDmg = Math.floor(incomingDmg * 0.5);
    player.currentHealth -= incomingDmg; updatePlayerHealthBar(); spawnFloatingText('player-combat-area', "-" + incomingDmg, "float-enemy-dmg");

    if (incomingDmg > 0) { for (let slotKey in player.equipment) { let equippedItem = player.equipment[slotKey]; if (equippedItem && equippedItem.onHitTaken) { let proc = equippedItem.onHitTaken; if (Math.random() < proc.chance) { applyStatus(e, proc.type, proc.duration, proc.type === 'blind'); spawnFloatingText(`enemy-${e.id}`, `${proc.type.toUpperCase()}!`, "float-enemy-dmg"); } } } }
    if (hasStatus(player, 'thorns') && incomingDmg > 0) { let tDmg = Math.floor(incomingDmg * 0.2); e.hp -= tDmg; animateHit(e.id, tDmg, false); }
    if (skillData && skillData.trigger === 'post_attack') { if (skillData.lifestealFraction && incomingDmg > 0) applyHeal(e, Math.ceil(incomingDmg * skillData.lifestealFraction)); if (skillData.drainPlayerATB && incomingDmg > 0) { player.attackProgress = 0; let pAtb = document.getElementById('player-atb'); if(pAtb) pAtb.style.width = '0%'; spawnFloatingText('player-combat-area', skillData.floatingText || "FATIGUED!", "float-miss"); } }
    if (skillData && skillData.summonEnemyId && skillData.trigger === 'attack') { let aliveCount = activeEnemies.filter(en => en.hp > 0 && !en.isDead).length; let maxAllowed = skillData.maxMinionsOnBoard || 5; if (aliveCount < maxAllowed) { summonEnemyMidBattle(skillData.summonEnemyId, skillData.summonHpOverride); } }
    
    let container = document.getElementById('game-container'); 
    container.style.transition = 'none'; container.style.boxShadow = 'inset 0 0 50px rgba(231, 76, 60, 0.8)'; 
    setTimeout(() => { container.style.transition = 'all 0.2s'; container.style.boxShadow = '0 0 20px rgba(0,0,0,0.5)'; }, 100);
    
    if (player.currentHealth <= 0) { triggerGameOver("Killed by " + e.name); }
}

function updatePlayerHealthBar() {
    let maxH = getMaxHealth(); let pPercent = Math.max(0, (player.currentHealth / maxH) * 100);
    ...
    if (pBar) { if (pPercent <= 30) pBar.style.backgroundColor = '#e74c3c'; else if (pPercent <= 50) pBar.style.backgroundColor = '#e67e22'; else if (pPercent <= 70) pBar.style.backgroundColor = '#f1c40f'; else pBar.style.backgroundColor = '#2ecc71'; }
}

function packDefeated() {
    if(waveManager.isUpgrading) return; waveManager.isUpgrading = true;
    if (isTestMode) { runStats.tempAtkActive = false; endRun('TEST CLEARED', '#2ecc71'); return; }
    runStats.tempAtkActive = false; player.activeEffects = []; renderStatusEffects();     
    let isBoss = getLevelAndWave().isBoss; let gemsEarned = 0; if (isBoss) gemsEarned = 5; else if (Math.random() > 0.8) gemsEarned = 1; if (gemsEarned > 0) addCurrency('gem', gemsEarned);
    updateCombatStatsPanel();

    if (isBoss) { 
        if (player.highestStageUnlocked === undefined) player.highestStageUnlocked = 0;
        let flatIndex = (waveManager.currentBiomeIndex * 6) + waveManager.currentSubstageIndex;
        if (flatIndex >= player.highestStageUnlocked && flatIndex < 59) { player.highestStageUnlocked = flatIndex + 1; }
        showBossClearUI(); return;
    } 

    let shopPool = []; window.currentShopPool = shopPool;
    commonUpgradesData.forEach(u => { let count = runStats.commonUpgradeCounts[u.id] || 0; let scaleMult = count + 1; let newEffect = {}; let newDesc = ""; if (u.id === 'heal') { newDesc = `Heal 25% Max HP`; } else if (u.id === 'temp_atk') { newDesc = `+200% P/M.Atk (1 Wave)`; } else if (u.id === 'vitality') { newEffect.hp = 25 * scaleMult; newDesc = `+${newEffect.hp} Max HP`; } shopPool.push({ ...u, rarity: 'common', cost: 0, effect: newEffect, desc: newDesc }); });
    runUpgradeData.forEach(u => { if (runStats.upgradeLevels[u.id] < u.maxLevel) { let cost = 1 + (runStats.upgradeLevels[u.id] * 2); shopPool.push({ ...u, rarity: 'uncommon', cost: cost, currentLvl: runStats.upgradeLevels[u.id] }); } });
...
function buyRunUpgrade(upgrade) {
    if (runStats.runes < upgrade.cost) return; runStats.runes -= upgrade.cost;
    if (upgrade.rarity === 'uncommon') { runStats.upgradeLevels[upgrade.id]++; if(upgrade.id === 'p_atk') runStats.pAtk += 20; if(upgrade.id === 'm_atk') runStats.mAtk += 20; if(upgrade.id === 'p_def') runStats.pDef += 10; if(upgrade.id === 'm_def') runStats.mDef += 10; if(upgrade.id === 'spd') { runStats.atkSpdMulti += 0.20; runStats.spdMulti += 0.10; } if(upgrade.id === 'crit') { runStats.crit += 0.05; runStats.critDmg += 0.50; } if(upgrade.id === 'splash') runStats.splashDmg += 0.10; if(upgrade.id === 'double') runStats.doubleHitChance += 0.10; if(upgrade.id === 'lifesteal') runStats.lifesteal += 0.05; if(upgrade.id === 'evasion') runStats.evasion += 0.05; } 
    else if (upgrade.rarity === 'common') { runStats.commonUpgradeCounts[upgrade.id] = (runStats.commonUpgradeCounts[upgrade.id] || 0) + 1; if (upgrade.id === 'heal') { let maxH = getMaxHealth(); applyHeal(player, Math.floor(maxH * 0.25)); } if (upgrade.id === 'temp_atk') { runStats.tempAtkActive = true; } if (upgrade.id === 'vitality') { runStats.maxHpBonus += upgrade.effect.hp; player.currentHealth += upgrade.effect.hp; updatePlayerHealthBar(); } }
...
function collectRunRewards() { 
    ...
    if (hStats) { 
        ...
        while(hStats.exp >= hStats.expNeeded) { hStats.level++; hStats.exp -= hStats.expNeeded; hStats.expNeeded = Math.floor(hStats.expNeeded * 1.5); player.talentPoints++; player.maxHealth += 25; leveledUp = true; } 
        if(leveledUp) showNotification(`🎉 Level Up! You are now Level ${hStats.level}!`); 
    } 
    document.getElementById('global-modal-backdrop').style.display='none';
    document.getElementById('run-summary-ui').style.display = 'none'; 
    document.getElementById('boss-clear-ui').style.display = 'none'; 
    openMenu('stages'); updateUI(); waveManager.transitioning = false; waveManager.isUpgrading = false; 
}

function updateUI() { document.getElementById('gold-amount').innerText = player.gold; document.getElementById('gem-amount').innerText = player.gems; if (player.heroStats && player.heroStats[player.currentHero]) { let activeH = player.heroStats[player.currentHero]; document.getElementById('player-level-text').innerText = activeH.level; document.getElementById('player-exp-fill').style.width = ((activeH.exp / activeH.expNeeded) * 100) + '%'; document.getElementById('player-exp-text').innerText = activeH.exp + '/' + activeH.expNeeded; } document.getElementById('tp-amount').innerText = player.talentPoints; let elDmg = document.getElementById('talent-lvl-damage'); let elGold = document.getElementById('talent-lvl-gold'); if(elDmg && player.talents) elDmg.innerText = player.talents.damage || 0; if(elGold && player.talents) elGold.innerText = player.talents.gold || 0; }
function generateRandomEquipment() { if (equipmentData && equipmentData.length > 0) { let baseItem = equipmentData[Math.floor(Math.random() * equipmentData.length)]; let statsCopy = {}; if (baseItem.stats) { for (let key in baseItem.stats) { statsCopy[key] = baseItem.stats[key]; } } return { name: baseItem.name, type: baseItem.slot, slot: baseItem.slot, icon: baseItem.icon, stats: statsCopy, onHit: baseItem.onHit || null, onHitTaken: baseItem.onHitTaken || null }; } const slots = ['head', 'body', 'legs', 'boots', 'weapon', 'leftHand', 'ring', 'amulet']; const slot = slots[Math.floor(Math.random() * slots.length)]; const prefixes = ['Rusty', 'Iron', 'Steel']; const prefix = prefixes[Math.floor(Math.random() * prefixes.length)]; const itemTypes = { 'head': { name: 'Helm', icon: '🪖', stats: ['pDef', 'mDef', 'maxHp'] }, 'body': { name: 'Armor', icon: '👕', stats: ['pDef', 'mDef', 'maxHp'] }, 'legs': { name: 'Greaves', icon: '👖', stats: ['pDef', 'spd', 'evasion'] }, 'boots': { name: 'Boots', icon: '🥾', stats: ['spd', 'evasion', 'mDef'] }, 'weapon': { name: 'Sword', icon: '🗡️', stats: ['pAtk', 'mAtk', 'atkSpd', 'crit'] }, 'leftHand': { name: 'Off-Hand', icon: '🛡️', stats: ['pDef', 'mDef', 'maxHp', 'mAtk', 'pAtk'] }, 'ring': { name: 'Ring', icon: '💍', stats: ['pAtk', 'mAtk', 'luck', 'crit'] }, 'amulet': { name: 'Amulet', icon: '🧿', stats: ['maxHp', 'luck', 'evasion'] } }; const itemDef = itemTypes[slot]; let fallbackStats = {}; let statName = itemDef.stats[0]; fallbackStats[statName] = 5; return { name: `${prefix} ${itemDef.name}`, type: slot, slot: slot, icon: itemDef.icon, stats: fallbackStats, onHit: null, onHitTaken: null }; }
